...
//
// 双密码完全分离（防白嫖 + 防权限蔓延）：
//   - CHAT_PASSWORD  → 仅供浏览器聊天页使用（/api/chat + /api/config）
//   - ACCESS_TOKEN   → 仅供 API 端点使用（/v1/chat/completions）
//   - 两者互不通用：CHAT_PASSWORD 泄露只能用聊天页，不能调 API；反之亦然
//
// 未设置的处理：
//   - 未设 CHAT_PASSWORD → 聊天页禁用（config.js 返回 chatEnabled=false，陌生人无法白嫖）
//   - 未设 ACCESS_TOKEN  → API 端点不强制鉴权（向后兼容，但生产环境务必设置）
//
// 这样即使忘记关 ENABLE_CHAT_PAGE，陌生人没有 CHAT_PASSWORD 也进不了聊天页。

// scope: 'chat' → 校验 CHAT_PASSWORD；'api' → 校验 ACCESS_TOKEN
// 返回 null = 通过；返回 Response = 拒绝（直接 return 该 Response）
export function checkAuth(env, request, scope = 'api') {
  const secret = scope === 'chat' ? env.CHAT_PASSWORD : env.ACCESS_TOKEN;
  if (!secret) {
    // chat scope 未设密码 = 聊天页禁用，直接拒绝（config.js 会单独处理禁用提示）
    if (scope === 'chat') {
      return new Response(
        JSON.stringify({ error: { message: '聊天页未启用（未配置 CHAT_PASSWORD）', type: 'authentication_error' } }),
        ...
        while (true) {
          ...
          while ((idx = buffer.indexOf('\n\n')) !== -1) {
            ...
            for (const line of block.split('\n')) {
              ...
              if (p.usage) pendingUsage = { ...pendingUsage, ...p.usage };   // 浅合并（anthropic message_delta 只更新 completion_tokens）
